...
chrome.runtime.sendMessage({type:'list',allList}, function(response) {
    if(chrome.runtime.lastError){}
});
if(videoCard.length == prevList.length && prevList.length != 0) return;
prevList = videoCard;
[...videoCard].forEach(card => {
    card.querySelectorAll(`.${className}`) && 
    [...card.querySelectorAll(`.${className}`)].forEach(element=>{
        element && element.remove();
    });
    var down = document.createElement('div');
    down.className = className;
    down.style.height = '30px';
    down.style.width = '40px';
    down.style.position = 'absolute';
    down.style.top = '10px';
    down.style.right = '10px';
    down.title = 'click me to download tiktok video!'
    var img = document.createElement('img');
    img.src = donwloadImgWhite;
    img.style.width = '100%';
    down.appendChild(img);
    card && card.appendChild(down);
    down.onclick = (e) =>{
        var videoSrc = card.querySelector('video') &&  card.querySelector('video').src || '';
        e.stopPropagation();
        e.preventDefault();
        if(videoSrc){
